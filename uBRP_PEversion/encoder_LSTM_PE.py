...
class GraphAttentionEncoder(nn.Module):
    def __init__(self, n_heads = 8, embed_dim=32, n_layers=3, max_stacks=4, max_tiers=4,  n_containers = 8, normalization = 'instance', ff_hidden = 512, LSTM_num_layers = 1):
        ...
        self.init_height = PositionalEncoding(embed_dim//2, 10+1, device=self.device)
        self.init_priority_encoding = PositionalEncoding(embed_dim//2, 100+1, device=self.device)
        self.init_block_embed = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self, x, mask=None):
        ...
        x = x.clone()
        binary_x = torch.where(x > 0., 1, 0).to('cuda:0') # Block -> 1 Empty -> 0
        ...
        x = x.to(torch.long).view(batch, max_stacks*max_tiers)
        PE_priority = self.init_priority_encoding(torch.zeros([batch, max_stacks*max_tiers]))[x].view(batch, max_stacks, max_tiers, self.embed_dim//2)
        
        h_x = torch.arange(max_tiers, 0, -1).view(1,1,max_tiers).repeat(batch,max_stacks,1).to('cuda:0') - stack_len.view(batch,max_stacks,1).repeat(1,1,max_tiers)
        h_x = torch.where(h_x < 0, 0, h_x)
        PE_height = self.init_height(torch.zeros([batch, max_stacks*max_tiers]))[h_x].view(batch, max_stacks, max_tiers, self.embed_dim//2)

        x = torch.cat([PE_priority, PE_height], dim=3)
