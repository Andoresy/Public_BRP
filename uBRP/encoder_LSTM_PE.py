...
class GraphAttentionEncoder(nn.Module):
    def __init__(self, n_heads = 8, embed_dim=32, n_layers=3, max_stacks=4, max_tiers=4,  n_containers = 8, normalization = 'instance', ff_hidden = 512, LSTM_num_layers = 1):
        ...
        self.init_height = PositionalEncoding(embed_dim//2, 10+1, device=self.device)(torch.zeros([batch, max_stacks*max_tiers]))
        self.init_priority_encoding = PositionalEncoding(embed_dim//2, 100+1, device=self.device)(torch.zeros([batch, max_stacks*max_tiers]))
    ...
    def forward(self, x, mask=None):
        ...
        PE_priority = self.init_priority_encoding[x].view(batch, max_stacks, max_tiers, embed_dim//2)
        ...
        PE_height = self.init_height[h_x].view(batch, max_stacks, max_tiers, embed_dim//2)
